...
@UseTestDataFrom(value = ConstantsHMH.CSV_FILES_BASE_PATH_HMH + "Test_REG063_QuickSearchTextFieldsWithinResults.csv", separator = ConstantsHMH.CSV_SEPARATOR)
	...
	public class Test_REG069_AllSitesAdvanceSearchTextFieldsWithinResultsforHIP extends com.hmh.automation.tools.AbstractBaseSiteTestHIP{
	...
	@Test
	@Title("Test_REG069 - All Sites Advance search Functionality HIP- TextFieldsWithinResults")
	public void test_REG069_AllSitesAdvanceSearch() {
    	...
    	if (isFileFound == false){
        	...
        	hmhDocumentLibrarySteps.createHipObject(name3+ConstantsHMH.TODAY_DATE, title, grade);
            hmhDocumentLibrarySteps.refresh();
            
            hmhDocumentLibrarySteps.checkIfAssetIsPresent(objectName+ConstantsHMH.TODAY_DATE);
            hmhDocumentLibrarySteps.hipobjectFromListClick(objectName+ConstantsHMH.TODAY_DATE);
...
//   		**** advance search for consumer Asset 
   		...
   		hmhSearchResultsSteps.isElementPresentInSearchResult(fileName+ConstantsHMH.TODAY_DATE, waitForSearchResultsSeconds, ConstantsHMH.SEARCH_RESULTS_MANUAL_SCROLL, true);   		  		  		
        shareHeaderSteps.customWait(5);
